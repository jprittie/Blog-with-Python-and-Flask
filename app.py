...
def register():
    if request.method == 'POST':
        ...
        userDetails['username'], userDetails['email'], password))
...
def login():
    if request.method == 'POST':
        ...
        if resultValue > 0:
            user = cur.fetchone()
            passwords_match = check_password_hash(user['password'], userDetails['password'])
            if passwords_match:
                session['login'] = True
                session['authorId'] = user['user_id']
...
def write_blog():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO blog(title, body, author_id) VALUES(%s, %s, %s)", (title, body, authorID))
        mysql.connection.commit()
        cur.close()
